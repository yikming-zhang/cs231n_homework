        ...
        for filter in range(F):
            for hh in range(H_out):
                for ww in range(W_out):
                    out[data_point, filter, hh, ww] = np.sum(w[filter,:,:,:] * xx[:,stride*hh:stride*hh+HH, stride*ww:stride*ww+WW]) + b[filter]
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
        ...
        for filter in range(F):
            for hh in range(H_out):
                for ww in range(W_out):
                    dw[filter,:,:,:] += dout[n,filter,hh,ww] * xx[:,stride*hh:stride*hh+HH, stride*ww:stride*ww+WW]
                    db[filter] += dout[n, filter, hh, ww]
                    dx_x[:, stride*hh:stride*hh+HH, stride*ww:stride*ww+WW] += dout[n,filter,hh,ww] * w[filter,:,:,:]
